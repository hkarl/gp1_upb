...
- Kalender: Klasse, die Datum bereit hält und auf ~tick~ einen Tag weiterzählt
...
- Natürlich Idee: Wir erben von zwei Klassen statt nur einer
...
- Methodenaufruf: eindeutig zuzuordnen 
  - Methodennamen der Oberklassen unterschieden sich!
...
- Regel: Die Methode der zuerst angebenen Klasse überdeckt die Methode der später angebenen Klasse
...
Was würde eigentlich passieren, wenn die Oberklasse im Konstruktur
~super~ benutzt? 
...
- Das hängt offenbar mit der Reihenfolge von ~C(A,B)~ vs. ~C(B,A)~
  zusammen
- Das müssen wir genauer anschauen! (Abschnitt [[Method Resolution Order]])  
...
allen bisherigen Regel in direktem Einklang. 


#+CAPTION: Gleiche Variable in mehreren Oberklassen -- Schreiben durch den ersten Konstruktor
#+ATTR_LaTeX: :width 0.75\linewidth
...
#+CAPTION: Gleiche Variable in mehreren Oberklassen -- Schreiben durch den zweiten Konstruktor
#+ATTR_LaTeX: :width 0.75\linewidth
...
B~. Diese Termoinologie ist bei Java entlehnt und bedeutet lediglich,
...
- Zwischen gleichrangigen Klasse, lass den Programmierer der
...
- Was sollte dann für die Linearisierung von ~Cneu~ gelten? 
...
- Wir schauen von der Klasse nach oben (und sehen keinen Baum, sondern
  einen allgemeinen Graph) 
...
#+CAPTION: Zwei äquivalent Darstellungen von ~class C(A,B)~: UML 
#+ATTR_LaTeX: :width 0.25\linewidth
#+NAME: fig:simpler-diagrams
[[./uml/simpler-graph-0.png]]

#+CAPTION: Zwei äquivalent Darstellungen von ~class C(A,B)~: Vereinfachtes Diagramm; Ziffern geben Reihenfolge der Oberklassen an
...
- C.b(): Methode B.b() aufgerufen, die wird wohl Y.m() aufrufen
  - Also inkonsistentes Verhalten der Klasse ~C~!
...
Wir brauchen also: 
- Ein Verfahren, das eine Linearisierung bestimmt wenn möglich
  - Im Einklang mit Monotonie und local precedence 
- Oder einen Fehler mitteilt falls nicht existent 

Aber zunächst: Noch ein paar Beispiele! 
...
- Lin(B) = B + merge(DO, EO, DO) = B + D + merge(O, EO, O) = B + D +
  E + merge(O, O, O) = BDEO
- Lin(C) = CDGO entsprechend 
- Lin(A) = A + merge(BDEO, CDGO, BC)
  - = A + B + merge(DEO, CDGO, C)
  - = A + B + D +  merge(EO, CDGO, C)
    - Wir fangen beim nächsten merge-Schritt wieder mit dem ersten
      Argument an, falls möglich
    - Das führt zu Tiefensuche
  - = A + B + D + E + merge(O, CDGO, C)
    - O wird aufgeschoben, da es Oberklasse von C ist; Linearisierung 
      von C muss erst eingebaut werden 
  - = A + BDE + CDG + merge(O, O, []) = ABECDGO
...
Ein unachtsames Umdrehen führt zum Ausführen anderen Methoden -- in
...
Klasse ~B~ aufgerufen um eine Methode ~m2~ aufzurufen, so ~m2~ in der
...
führen zu Python-typischen Programmiermodell: 
...
- Beispiel: dependency injection
  - Wir ändern das Aufrufverhalten einer durch Ableiten! 
...
#+BEGIN_SRC python :exports both :results output
  class Teigfabrik:
      def get_teig(self):
          print("Teig aus genamnipuliertem Weizen") 
...
#+RESULTS:
: Teig aus genamnipuliertem Weizen
...
- Pizzeria-Beispiel zeigt typisches Muster:
  - Durch geschicktes Ableitung  von zwei Klassen wurde Funktionalität
    vereinigt
...
- Mixin: Prima für Reuse fertigen Codes gedacht, nicht zur Spezialisierung
- Vorgehen:
...
- Es gibt leicht zu beherrschende Fälle: Oberklasse sind disjunkt
