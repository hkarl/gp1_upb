...
- Eine Auswirkung: Verteilung eines Programms auf Dateien
  - Regel: Eine Datei = eine (öffentliche) Klasse 
...
- Java unterscheidet die Übersetzung und die Ausführung eines
  Programms 
- Eine ~*.java~ Datei ist nicht direkt ausführbar
- Compiler: ~javac~
  - Übersetzt von ~*.java~ nach ~*.class~
  - ~*.class~ Binärdatei mit Zwischencode; nicht direkt für Menschen
    lesbar; nicht direktausführbar 
- Ausführung: ~java~
...
** Übersetzen und Starten eines Programms -- Beispiel 
...
- Jupyterhub ist nur bedingt auf die Ausführung von Java ausgelegt
...
- IDEs: [[https://netbeans.org][NetBeans]],  [[https://eclipse.org/][Eclipse]], [[https://www.jetbrains.com/idea/][IntelliJ IDEA]], [[https://developer.android.com/studio/index.html][Android Studio]], [[http://www.bluej.org][BlueJ]] (zum Lernen
  ...
  - Aber tun Sie sich das für diese VL  nicht an!
...
- Im Beispiel:
  - ~class~ ist vertraut
    - Danach sieht das aus wie eine Methodendeklaration?
...
- Durch die Angabe von Typen bei Parameter und Rückgabewert bekommt
  die Methode selbst eine (Sammlung von) Typen
...
- Ein Kommentar über eine oder mehrere Zeilen, mit ~/*~ und ~*/~
  eingerahmt 
...
- Java benutzt Variablen!
  - Ein Name für eine Speicherstelle
  - Die mit Werten eines bestimmten Typs gefüllt werden kann
...
Ihr Name und ihr Typ werden in einer Anweisung angegeben. 
...
- Es fehlt die umgebende Klasse; es gibt keinen Code ausserhalb einer
  Klasse 
...
- Vielmehr speichern sie einen Zeiger (eine /Referenz/) auf die Speicherstelle
...
- Java benutzt unterschiedliche Semantik von Variablen 
  - Einfache Variablen: Schachtel mit Wert
    - Sog. /primitive types/
    - ~byte~, ~short~, ~int~, ~long~, ~float~, ~double~, ~boolean~, ~char~
  - Variablen komplexer Typen: Schachtel mit Zeiger (Referenz) auf Wert
    - Sog. /reference types/
...
- Jedem konstanten Wert kann ein Block zugeordnet werden
  - Wird ausgeführt, wenn der Vergleich richtig ist 
...
- Grundstruktur von Java-Programmen: Alles in einer Klasse
  ...
  - Programm kann aus mehreren Dateien, Klassen bestehen
