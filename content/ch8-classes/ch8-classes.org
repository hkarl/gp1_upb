...
#+BEGIN_LaTeX
Abbildung~\ref{fig:uml-ellipse} zeigt ein sog. UML-Diagramm für eine Klasse. UML steht für Unified Modelling Language und ist eine Standard-Notation für Klassen (und Objektorientierung im allgemeinen; siehe später). 
...
*** UEBUNG Wietere Beispiele für Klassen ausdenken 


- Fahrzeug 
- Andere Typen in Malprorammen 
- Buch 
- Artikel in Webshop
- usw. 

(Nach keine Hierarchien, kommt später) 
...
*** UEBUNG Beispiel-Instanzen fpr die zuvor ausgedachten Klassen 
...
*** UEBUNG Banale Frage: muss Konstruktor explizit aufgerufen werden ? 

*** UEBUNG Was passiert, wenn man dem Konstruktur einen anderen Namen ghibt als __init__? 

*** UEBUNG Nicht trivial: was passiert, wenn man den Konstruktor von Hand tortzdem aufruft? 

Das ist gar nicht so einfach; insbes. im folgenden Kapitel mit
Vererbung wird das kompliziert. 
...
Im nächsten Schritt 
#+BEGIN_LaTeX
(Abbildung~\ref{fig:instantiate-before-constructor}),
#+END_LaTeX
...
Sobald die Zeile 3 ausgeführt wurde
#+BEGIN_LaTeX
(Abbildung~\ref{fig:instantiate-first-attribute}),
#+END_LaTeX
wurde das erste Attribut (der Name
...
#+CAPTION: Instatiieren des Objekts; Anlegen des ersten Attributes
#+ATTR_LaTeX: :width 0.75\linewidth
...
Dies hat vor allem historische Gründe und Gründe in der Analogie zu
...
Nachdem Zeile 6 ausgeführt wurde 
#+BEGIN_LaTeX
(Abbildung~\ref{fig:instantiate-constructor-done})
#+END_LaTeX
sind die weiteren Attribute ~y~,
...
Objekt
#+BEGIN_LaTeX
 (Abbildung~\ref{fig:pt-instantiate-ready}). 
#+END_LaTeX
...
Das ist eigentlich etwas seltsam: Der Konstruktor hatte doch ~None~
...
#+BEGIN_LaTeX
Abbildung~\ref{fig:pt-instantiate-ready-compact}
#+END_LaTeX
noch die kompakte Darstellung des
Endzustandes (inhaltlich äquivalent zu
#+BEGIN_LaTeX
 Abbildung~\ref{fig:pt-instantiate-ready}
#+END_LaTeX
).
...
*** UEBUNG Was passiert, wenn der erste Parameter nicht ~self~ heißt? 

Aber: SEHR starke Konvention!! 
...
- Diese drei Objekte haben jeweils die gleichen Attribute; diese
  Attributnamen referenzieren jeweils die gleiche Werte
...
#+CAPTION: Drei Ellipsen-Objekte, kompakt
#+ATTR_LaTeX: :width 0.75\linewidth
#+NAME: fig:pt-three-ellipses-compact
[[./figures/pt-three-ellipses-compact.png]]


*** UEBUNG Andere Klassen definieren , andere Objekte instantiieren 
...
es Ihnen vielleicht seltsam vorkommen, dass wir keine Attribute an sich
...
*** UEBUNG Das dict von Ellipse ausgeben lassen 

*** UEBUNG Dem dict von Ellipse einen weiteren Wert hinzufügen 
...
#+RESULTS:
: <bound method Simple.f of <__main__.Simple object at 0x1010355c0>>
: f aufgerufen
...
*** UEBUNG Was passiert, wenn mit mehreren Objekten aufgerufen wird? 
...
*** UEBUNG Für Ellipse lt implementieren? Erklären, warum das komisch ist? 

*** UEBUNG plus Operator überladen 

zwei Ellipsen addieren?? oder ein Plausibles Beispiel? Schnitt? 

*** UEBUNG Was schweres? Einen Iterator auf einer Ellipse implementieren? 

Den Rand ablaufen? in vorgegeben Schrittweite? 

*** UEBUNG Slicing einer Ellipse? Mit wInkeln? 
...
*** UEBUNG Was passiert hier und warum? 
...
*** UEBUNG Beiepiele: woezu braucht man Klassenattribute/methoden? 
...
#+CAPTION: Erweiterung des Attribute-dict um weitere Namen/Werte
#+ATTR_LaTeX: :width 0.75\linewidth
#+NAME: fig:x-inattributedict
[[./figures/pt-x-in-dict.png]]


*** UEBUNG Zusammenfassung: Wie können Attribute einer Instanz erzeuigt werden? 

Wichtig: geht von innerhalb des Scopes der Klasse wie auch sonst --
immer wenn eine Referenz vorhanden ist 

*** UEBUNG Atrtibute der Instanz im Block der Klasse definieren? 

Erzeugt das hier ein Attirbut der Instanz? Warum? 
...
Bei aller Ähnlichkeit zwischen ~def~ und ~class~ ist dies ein
entscheidender Unterschied! ~def~ als Anweisung führt *keine*
Anweisungen des Blocks aus; es wird nur der Name der Funktion angelegt
und mit dem Funktionscode verbunden. ~class~ andererseits führt die
Anweisungen aus, die im Block der Klasse enthalten sind! 
...
Anmerkung: die Zeichenkette ~y~ für das Attribut der beiden Objekte
...
*** UEBUNG @classmehotd vs. @staticmethod würde hier passen, aber wir haben noch keine Dekoratoen :-(

das müssen wir später ggf. nachholen? oder überspringen? 
...
*** UEBUNG Weitere Beispiele von Klassen mit statischen und dynamischen Attributen 


** Zusammenfassung: Dynamisch vs. statisch Attribute 

#+ATTR_LATEX: :align p{0.3\textwidth}|p{0.3\textwidth}|p{0.3\textwidth}
...
*** UEBUNG Das dict der Klasse (nicht eine objektes) anschauen
...
*** UEBUNG Was ist also der Unterschied zwischen einem Klassenobjekt und einem Instanzobjekt? 
...
*** UEBUNG Wie würde man den Zugriff auf die Parameter machen, deren Namen im Foramt-String angegeben ist? 

das macht man per kwargs! 
...
*** UEBUNG Heap, oder Queue, oder ... implementieren 
...
zwei Punkt-Objekte. Das erste wird ebenfalls durch den Namen ~p~ (im
...
*** UEBUNG REchteck Klasse definieren 
...
*** UEBUNG Abstand zweier Rechtecke bestimmen 

*** UEBUNG Abstand einer Liste von Rechtecken bestimmen 

*** UEBUNG Punktklassen umstruktureien: abstand benutzt abstand2, oder umgekehrt? 
...
*** UEBUNG Studierendenverzeichnis 

*** UEBUNG Vorlesungeverzeichnis 
...
*** UEBUNG Code umstrukturieren : Echte rekursive Struktur 

Baum hat zwei Bäume als Kinder. Und dann Vort- Nachteile diskutieren  
...
- Wenn keine Kinder! 
- Wurzel braucht allerdings Sonderbehandlung, wenn keine Kinder 
...
*** UEBUNG Größeres Beispiel: Eine Personendatenbank? 


*** UEBUNG Größeres Beispiel: PAUL? 
Eigentlich braucht das aber inheritance
