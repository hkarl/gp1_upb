...
bezogen. Es ist sinnlos, von einem Objekt ohne Bezug auf /seine/
Klasse zu reden.
...
Methode ~__init__~. Abbildung [[fig:instantiate-classname]] zeigt diesen
Zustand. 


#+CAPTION: Anlegen der Klasse selsbt 
...
Sobald die Zeile 3 ausgeführt wurde (Abbildung ), wurde das erste
Attribut (der Name ~x~) im Ellipse-Objekt angelegt. In ~Ellipse
instance~ ist ein Pfeilpaar entstanden; der erste Pfeil weisst auf den
String ~x~ -- den Namen des Attributes -- der zweite Pfeil auf den
Wert 1.  Natürlich zeigt auch der Parameter ~x~ auf den Wert 1. 
...
unübersichtlich), wie sich  die Delegation der Repräsentation des
