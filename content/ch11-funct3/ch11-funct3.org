#+TITLE: Kapitel 11: Funktionen (Teil 3): Verschachtelung und funktionale Programmierung 
...
** Beispiel: Quadratzahlen aufsummieren 

Ziel: Funktion, die die Quadrate der ganzen Zahl von 1 bis $n$
aufsummiert 
...
** Beispiel: Kubikzahlen aufsummieren 

Ziel: Funktion, die die dritten Potenzen der ganzen Zahl von 1 bis $n$
aufsummiert 
...
Es gilt: $ \pi/8 = \frac{1}{1\cdot3}+ \frac{1}{5\cdot7}+ \frac{1}{9\cdot11}+ \cdot$

Ziel: Funktion, die das approximiert 

#+BEGIN_SRC python 
def pi_achtel (n):
    return sum([ 1/( (2*i-1) + (2*i+1) ) for i in range(1, n+1) ])
#+END_SRC

** Struktur? 

- Offenbar ähnliche Struktur 
- Warum immer den gleichen /Rahmencode/ neu schreiben?
  - Veränderliche Teile identifizieren? Und nur die neu schreiben? 

*** Andere Beispiele 

- Nullstellen unterschiedlicher Funktionen berechnen (z.B. mit
  Newton-Verfahren, ...)
- Daten anhand unterschiedlicher Kriterien sortieren
- Daten aus unterschiedlichen Datenquellen auslesen, mit
  unterschiedlichen Zugriffsfunktionen 
- ... 
...
#+CAPTION: Verschachtelte Funktionen, Anlegen der äußeren Funktion
...
ganz normaler Funktionsaufruf; hier passiert nichts außergewöhnliches
...
** Lesender Zugriff auf Namen der äußeren Funktion? Direkt? 
...
- Offenbar wird im Namensraum der  *umgebenden*  von ~g~ gesucht! 
- Und dort ein ~x~ gefunden 

** Schreibender Zugriff auf Namen der äußeren Funktion? Direkt? 
...
- ~x~ in Zeile 3 ist lokal zu ~g~! 
- Analog zu ~global~: Einfach /fremden/ Namen verändern ist schlechte
  Praxis

*** Fehlermeldung						  :dropslide:

Scheitern ist plausibel: Nach bisherigen Regeln wird an ~x~
zugewiesen, also ist ~x~ im *lokalen*
...
** Schreibender Zugriff auf Namen der äußeren Funktion? Reihenfolge? 

Liegt es vielleicht an der Reihenfolge? ~x~ in ~f~ zuweisen, bevor in
~g~ benutzt? 
...
- Scheitert ebenfalls, nach gleicher Logik! 
...
- Analog zu ~global~ für globale Namen? 
- (Und ja, das ist nicht unbedingt gute Praxis...) 
...
pingo_title = "Was wird wird ausgegeben?" 
pingo_questions = ["h: None, g: 42, f: 1", "h: 43, g: 42, f: 1", "h: 2, g: 42, f: 2", "h: 43, g: 43, f: 1", "h:43, g:43, f:43"]
pingo_duration = "180"
...
- einerseits natürlich ein Name  ~x~ im Frame ~f1~ von ~f~ gibt,
  die auf den Wert 1 verweist;
- anderseits auch ein Name  ~x~ im Frame ~f2~ von ~g~ gibt, die
  auf den Wert 43 (nach der Addition) weist;
- aber /kein/ Name ~x~ im Frame von ~h~ gibt; ~nonlocal~ sorgt
...
#+CAPTION: Nachschlageregel LEGB 
#+ATTR_LaTeX: :width 0.5\linewidth
#+ATTR_LaTeX: :caption \caption{Nachschlageregel LEGB  \label{fig:legb}}
#+NAME: fig:legb
[[./uml/legb.png]]


#+BEGIN_LaTeX
Abbildung~\ref{fig:legb} illustriert die Nachschlageregel LEGB: von innen nach aussen. 
#+END_LaTeX
...
#+CAPTION: Aufruf einer Funktion, die als Funktionsergebnis bekannt wurde
#+ATTR_LaTeX: :width 0.75\linewidth
...
#+CAPTION: Zurückgegebene Funktion mit Parameter: Vor Aufruf
#+ATTR_LaTeX: :width 0.75\linewidth
...
- Lokale Namen:
  ...
  - Namen, an die in der Funktion zugewiesen wurde
- Nicht-lokale Namen:
  - Namen aus enclosing scopes - egal ob mit nonlocal Schreibzugriff
    erlaubt oder nicht  
- Globals bleiben globals! 
...
Namensraum, in dem die fehlenden Namen (hier: ~n~) gesucht werden
...
Je nach Literaturquelle wird auch das Programmiermuster selbst als
Closure bezeichnet, nicht nur die Realisierungstechnik. Das macht
letztlich keinen großen Unterschied. 

*** Anmerkung: Vorkommen von Closures 				  :dropslide:

Closures kommen häufig in funktionalen Sprachen vor. Neben Python ist
Javascript ein populäres Beispiel, bei dem häufig Closures benutzt
werden, um die Reaktion auf Nutzereingaben oder Daten vom Server zu
beschreiben. Auch in Sprachen wie Lisp oder Ruby sind Closures ein
populäres, elegantes Sprachmittel. 

Sprachen wie Java, C++, C# und Abkömmlinge stellen in der Regel nur
vereinfachte Strukturen zur Verfügung und können das Konzept der
Closures nicht vollständig abbilden.  
...
*** UEBUNG In closures die gemerkte Namen verändern? 
...
*** UEBUNG Closures nutzen, um C static Namen nachzubauen?
...
- /Ausdruck/, der eine /anonyme Funktion/ vereinbart
  - Keine Anweisung! 
  - Präziser: Ausdruck, der ein Funktionsobjekt erzeugt und eine Referenz auf
...
** Lambda in Closures - Vorsicht! 

Lambda-Funktionen binden freie Variablen zu einem späteren Zeitpunkt
als /normale/ Funktionen 

*** Closure mit normaler Funktion 
...
** Mögliche Abhilfe: Lambda-Parameter explizit binden

#+BEGIN_SRC python
numbers = 'one', 'two', 'three'
funcs = []
for n in numbers:
    funcs.append(lambda nn=n: print(nn))
...
** \lambda als key-Funktion 

- Viele Funktionen, die Daten vergleichen, haben einen Parameter ~key~
- An ~key~ kann ein \lambda-Ausdruck übergeben werden, die aus einem
  Datum den für den Vergleich relevanten Teil extrahiert 

*** Beispiel 

- Alphanumerische Daten unterschiedlich sortieren 
- von https://realpython.com/python-lambda/
...
** \lambda in GUI Programmierung 

- Szenario: Sie erzeugen eine Gruppe von Buttons für eine grafische
  Benutzerschnittstelle
- Wenn ein Knopf gedrückt wird, soll eine Funktion mit einem Parameter
  aufgerufen werden 
- Beispiel hier: [[https://docs.python.org/3/library/tk.html][Tk]] (von
  https://stackoverflow.com/questions/890128/why-are-python-lambdas-useful)
  - Alternative: Für jeden Button eine eigene Funktion
    definieren.... Unübersichtlich!  

#+BEGIN_SRC python 
def my_callback(arg): pass 
...
** \lambda in Web-Anwendungen 

(Typischerweise: Javascript Code!) 

- Szenario:
  - Anwendung in Web-Browser schickt, in Schleife, mehrere
    Anfragen an Server
  - Sobald Antwort von Server eintrifft, soll jeweils eine andere
    Funktion aufgerufen werden, mit Parametern vom Server
...
- Komplexere Code-Strukturen
  ...
  - Webframework (Beispiel: Django)
    ...
    - Beispiel: ~@login_required~ für Methoden, die URL-Requests verarbeiten
- Ein Cache für aufwändige Funktionsaufrufe 
...
- Klassen können dekoriert werden 
- Beispiel: ~functools.total_ordering~
  - Stellt Klasse *eine* Vergleichsmethode (z.B. ~__lt__~) bereit,
    ergänzt der Dekorator die fehlenden Methoden
...
: Anfang f1
: Ende f1
: f1 (args: (0.2,), kwargs: {}) verbrauchte 0.20515108108520508 Sekunden
: Anfang f1
: Ende f1
: f1 (args: (0.6,), kwargs: {}) verbrauchte 0.603410005569458 Sekunden
...
Siehe auch: [[http://www.programiz.com/python-programming/decorator][Quelle des Beispiels]]
