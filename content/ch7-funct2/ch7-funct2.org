...
** Illustration -- Code ersetzt (Schritt 1) 
...
# wir ersetzen den Aufruf von f mit dem Code von f
# und benennen x um: 
print("in f, vor g")
f_x = g()
print("in f, nach g", "x: ", f_x)
#+END_SRC 

** Illustration -- Code ersetzt (Schritt 2) 
...
# wir ersetzen den Aufruf von f mit dem Code von f
# und benennen x um. 
# wir ersetzen ZUSÄTZLICH den Aufruf von g und benennen y um: 
print("in f, vor g")
# hier g ersetzen: 
g_y = 42 
g_return_value = g_y
f_x = g_return_value 
# vernichte g_y, g_return_value 
print("in f, nach g", "x: ", f_x)
#+END_SRC 
...
- Fehlermeldung: ~NameError: name 'ff' is not defined~
...
*** Beispiel 1: Name vor Funktionsdefinition zugewiesen 
...
*** Beispiel 2: Name *nach* Funktionsdefinition zugewiesen 
#+BEGIN_SRC python :exports both :results output
  def f():
      y = x 
      print(y)
...
~f~ die Name ~x~ im aufrufenden Scope gefunden hat und die eigene
Name ~y~ den Wert von ~x~ referenziert. 

#+CAPTION: Name ~x~ wird im globalen Scope gefunden, ~y~ referenziert den Wert von ~x~
...
Name ~x~ hier als eine Name, die lokal zur Funktion ist,
aufgefasst. Die Tatsache, dass es einen globalen Name ~x~ gibt,
spielt jetzt keine Rolle -- anders als im Beispiel oben. 

Offenbar liegt der Unterschied zwischen diesen beiden Beispielen
daran, dass an die Name ~x~ im zweiten Beispiel zugewiesen wird;
im vorherigen Beispiel wird ~x~ lediglich gelesen. 

Damit haben wir einen entscheidenen Punkt erarbeitet: Der Ort einer
*Zuweisung* an eine Name bestimmt ihren Scope. 



*** UEBUNG Ein paar analoge BEispiele bauen: Zuweisung an Name bestimmt Scope 
...
** Illustration durch Code-Ersetzung (Schritt 1) 

Schreiben wir diese Beispiel wieder durch Code-Ersetzung gedanklich
um: 
#+BEGIN_SRC python :exports both :results output
def f():
    x = 17
...
a = 42
# Aufruf f ersetzen, x umbenennen:
f_x = 17
g()
#+END_SRC 
...
** Illustration durch Code-Ersetzung (Schritt 2) 

Ersetzen wir den Aufruf von ~g~ -- welche Namen umschreiben?? 
...
f_x = 17
# Aufruf g ersetzen
# Jetzt Frage: z, a, x mit g_... umschreiben? 
# Welche sind im Scope von g? 

#+END_SRC 


** Illustration durch Code-Ersetzung (Schritt 3) 

Regel: *Zuweisung* an Namen bestimmt Scope! 
- Also: in ~g~ wird nur ~z~ zugewiesen, nur ~z~ ist im Scope.
- Nur ~z~ wird zu ~g_z~ umgeschrieben! 
...
# Aufruf f ersetzen, x umbenennen:
f_x = 17
# Aufruf g ersetzen, nur z zu g_z umschreiben! 
g_z = a 
g_z += x   # !!! 
print(g_z) 
...
Im obigen Progerammbeispiel tauchen drei verschiedene Namen
auf: alle ~x~ . Sie existieren in drei verschiedenen
...
#+CAPTION: Drei verschiedene Namen  ~x~ in drei verschiedenen Scopes
#+ATTR_LaTeX: :width 0.75\linewidth
...
Namen ~x~. Daher schaut die Funktion ~g~ beim Zugriff auf ~x~ nach,
...
Zuweisung an den lokalen Namen ~x~. 
...
*** Zuweisung erzeugt lokalen Namen  
...
** Modifikation des Wertes eines globalen Namens  
...
*** UEBUNG Beiepiele, bei denen wir dne globalen Namen  zum zurückgeben von Funktionswerte benuten 
...
** Wechselseitiger Aufruf: Code Ersetzung? 

Kommen wir bei diesem Beispiel mit Code-Ersetzung für die
Funktionsaufrufe weiter? 
- Problem: Welches ~x~? 
...
Funktionsaufrufe weiter? 
- Problem: Welches ~x~?
- Zeigt das hier das korrekte Verhalten???
...
Scope dieser beiden neuen Namen ~x~ und ~y~ ist aber der gleiche
...
*** Jeder Funktionsaufruf hat einen eigenen Namensraum 	       :B_definition:
    :PROPERTIES:
    :BEAMER_env: definition
    :END:
...
** Wechselseitiger Aufruf: Code Ersetzung richtig! 

Das Beispiel von oben mit *separaten* Namenspräfixen für jeden Aufruf: 
...
g_1_x = 2 
# Aufruf f ersetzen: 
f_2_x = 1 # !! zweiter Aufruf von f; neuer Namenspräfix! 
# Aufruf g ersetzen: 
g_2_x = 2 # entsprechend für g 
# Usw: 
f_3_x = 1 
g_3_x = 2 
# ... 
...
- Einsicht: Man kann eine Lösung nicht programmieren wenn man das
  Problem nicht versteht
  - Wissen über die Domäne des Problems
- Dann: Problem in einzelne Schritte zerlegen
  - Diese Schritte weiter zerlegen
- Dabei: Gute Zerlegungen haben geringe Abhängigkeiten zwischen
  Einzelschritten
- Vergleich: Alltagsbeispiele
  - Kuchen backen; morgens anziehen; ...
- Denken Sie nicht zu schnell in Code! Denken Sie innerhalb des
  Problems! 

** Funktionen und Coupling 

*** Eingabe mit Argumenten, Ausgabe mit return  
...
*** Globale Namen nur wenn nicht anders möglich                     :animate:
