...
Bei einer *Zuweisung* wird ein *Name* mit einem Wert verbunden; ein
solcher Name ist ein *Bezeichner*.  Der Bezeichner ist gebunden an den
Wert.
...
Gemeint: 
- wenn Variable *rechts* von ~=~: ein Bezeichner, der an einen Wert gebunden *ist*
- wenn Variable *links* von ~=~: ein Bezeichner, der an einen Wert gebunden *wird*
...
- Manche Namen haben Sonderrolle, dürfen nicht als Bezeichner benutzt werden 
...
ist ein Zeichenfolge, die in einer Programmiersprache eine Sonderrolle
...
- Für eine Folie mit einem Bezeichner: naja
...
*** Namenskonvention für Bezeichner in Python             :B_example:animate:
    :PROPERTIES:
    :BEAMER_env: example
    :END:
...
- Einfach: Bezeichner als Ausdruck benutzen
...
** Weitere Bezeichner -- Beobachtungen 
...
a = 42
b = 42
# a wird an einen neuen Wert gebunden ("rebinding") 
a = "Hallo"
#+END_SRC
...
* Arbeiten mit Bezeichnern und Zuweisungen 


** Bezeichner  an Bezeichner zuweisen 
...
- Allgemein:
  ...
  - Bezeichner wird zu Namen für diesen Wert
  - Egal, ob der Ausdruck rechts ein Bezeichner ist -- es geht um *den
    Wert* 
...
angezeigt. Rechts sieht man die an Bezeichner gebundenen Werte.
...
1. ~a = 5~ : Bezeichner ~a~ referenziert den Wert 5
2. ~b = a~ : Der Wert des Ausdrucks auf der rechten Seite wird
   bestimmt. Bezeichner ~b~ wird zu einem Namen für diesen Wert. 
3. ~a = a + 2~ : 
...
** Ausdrücke  mit Bezeichnern 

- In Ausdrücken können Bezeichner wie Werte benutzt werden 
- Bei Berechnung des Ausdrucks: Bezeichner wird durch gebundenen Wert
  ersetzt 
  - Wie Beispiel oben: ~b = a~ 
- Bezeichner links von ~=~ wird zu Namen für berechneten Wert 
...
** Beispiel: Gebundene Werte zweier Bezeichner vertauschen 

Wie vertauschen Sie die Werte zweier Bezeichner? 
...
*** UEBUNG Bezeichner-Werte tauschen ohne Hilfsvariable; xor 
...
Python: *Gleichzeitige* Zuweisung an mehrere Bezeichner möglich 

*** Syntax 

- Links von ~=~: Mehrere Bezeichner, durch Komma getrennt 
...
Bei einer Zuweisung werden ein oder mehrere *Bezeichner* mit einem oder
mehreren Werten verbunden.  Der Bezeichner *bindet an* den entsprechenden
Wert. 

*** Schreibweise 

- Zuweisung wird durch ein Gleichheitszeichen angegeben
- Links davon: der oder die *Bezeichner*, durch Komma getrennt
...
- Häufig: an Bezeichner einfachen arithmetischen Ausdruck aus diesem
  Bezeichner zuweisen
  - ~x = x + 1~, ~y = y / 2~
- Dafür Kurzschreibweise:
  - ~Bezeichner Operator= Ausdruck~
  - Entspricht: ~Bezeichner = Bezeichner Operator Ausdruck~
- Beispiele
  - ~x += 1~
  - ~y /= 2~
...
- Bezeichner?
  - Bezeichner ist nur Name für Wert, verweist auf  Wert
- *Typ des Bezeichners*: Typ des zugewiesenen Werts
  - Welcher Typ sollte es auch sonst sein?? 


** Bezeichnertyp veränderlich? 

Heißt das, dass sich der Typ eines Bezeichners verändern kann? 
...
- Typ-Information liegt *nicht* beim Bezeichner
  - Für eine Bezeichner *muss kein Typ angegeben werden*
...
- Ein Programmierer sieht einem Bezeichner (zunächst) nicht an, was
  darin steckt 
...
Typen-Informationen für */Bezeichner/* explizit im Programm-Code angegeben werden
...
Kommentar Pfahler: nicht bei OO - zu "auf Repräsentation zur laufzeit
verzichtet" XXX
...
- Zuweisungen geben Werten einen /Namen/, einen /Bezeichner/
