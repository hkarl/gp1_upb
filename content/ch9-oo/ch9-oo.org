...
#+ATTR_LaTeX: :caption \caption{Klassenhierarchie für Artikel: Daten in UML  \label{fig:artikel-uml-daten}}
...
#+BEGIN_LaTeX
Abbildung~\ref{fig:artikel-uml-daten} zeigt die UML-basierte Visualisierung
...
ist üblich und deute eine Geschwister-artige Beziehung an. 
...
Objekte.
#+BEGIN_LaTeX
Abbildung~\ref{fig:venn} illustriert das.  
#+END_LaTeX
...
#+ATTR_LaTeX: :width 0.5\linewidth
#+ATTR_LaTeX: :caption \caption{Klassenhierarchie für Artikel: Neue Methoden   \label{fig:artikel-new-methods}}
...
#+BEGIN_LaTeX
In Abbildung~\ref{fig:artikel-new-methods} werden die \emph{zusätzlichen}
...
*** UEBUNG Warum muss man den Konstruktor der Oberklasse überhaupt aufrufen? wird doch autoamtisch aufgerufen? 

Nein, nur der ERSTE Konstruktor-Aufruf passiert automatisch!
~__init__~ der Klasse des Objekts selbst. Oberklasse konstruktoren
werden nicht automatisch gerufen. 
...
*** UEBUNG DRY: Wie erweitern Sie eine Funktion der Oberklasse, ohne sie komplett ersetzen/neu schreiben zu müssen? 
...
beim Aufruf des Konstruktors: ~super~ bestimmt die Oberklasse und
...
*** UEBUNG Warum vererben und überschreiben; warum nicht einfach code kopieren? 
...
: Objekt von B; is-a A: True; is-a B: True; is-a C: False
: Objekt von B1; is-a A: True; is-a B: True; is-a C: False
: Objekt von B2; is-a A: True; is-a B: True; is-a C: False
: Objekt von C; is-a A: True; is-a B: False; is-a C: True
: Objekt von C1; is-a A: True; is-a B: False; is-a C: True
: Objekt von C2; is-a A: True; is-a B: False; is-a C: True
: Objekt von C3; is-a A: True; is-a B: False; is-a C: True
...
- Aber für Klassennutzer nicht schön zu benutzen
  - Eigentlich will man die Attribute wie gewohnt nutzen *ohne* Methoden aufzurufen 
  - Beispiel:
    - ~t1.temperature = 17~
...
Im Konstruktor wird in Zeile 4 auf die Property ~temperature~
...
*** UEBUNG In einer existierende KLasse mit Nutzungscode eines Namen zu einer property ändern 
...
In Sprachen wie Java ist der Begriff des Interfaces von wesentlich
größerer Bedeutung als in Python. Dies liegt insbesondere daran, dass
Java kein multiple inheritance beherrscht und versucht, dies über den
...
** Erben oder Instantiieren? 

- Vererben: Hinzufügen von *neuen* Attribute, Methoden 
- Instantiieren: Eine Objekt einer Klasse erschaffen und die
  abstrakten Attribute mit konkreten Werten füllen
  - Sandförmchen vs. Sandkuchen; Muster vs. Beispiel 
- (Leider) manchmal Klausurfehler: Statt eine Klasse zu einem Objekt
  zu instantiieren und die Werte zu setzen wird für jedes Objekt eine
  neue Unterklasse erzeugt
  - Das ist *Unsinn* 
...
*** UEBUNG PAUL extended 

Vererbung von Personen; polymorphism 
