...
// aber das Objekt geht sofort verloren; wir müssen eine Referenz darauf aufheben 
new Ellipse();
...
Dieser zweischrittige Prozess kommt Ihnen -- der Python-Perspektive --
...
Der Typ des Rückgabewerts (ggf. ~void~) wird nicht als Teil der
Signatur angesehen. Es gibt Programmiersprachen, bei denen auch das
...
#+BEGIN_SRC java :exports code :results output :classname  GlobalVariablesClass
  class Ellipse {
      // Daten deklarieren: 
      double x, y;
      double achse1, achse2;
...
Beispielsweise ist die Sichtbarkeit einer Klassenvariable die
Klasse. Für sie gilt zudem eine Ausnahme: Sie sind auch schon *vor*
ihrer Deklaration in allen Methoden der Klasse sichtbar.
...
** Sichtbarkeit zwischen Klassen: ~final~  


- Man kann das Überschreiben von Methoden verbieten
- Annotation: Schlüsselwort ~final~
- Keiner weiß, wozu das gut sein soll
  - Aber die Sprache sieht es vor 
...
- Mit ~final~ markiertes Datenattribut: eine Konstante
...
- Klasse mit mindestens einer abstrakten Methode ist eine /abstrakte
  Klasse/
  - Weil das aber zu einfach wäre: auch die Klasse muss ~abstract~
    markiert sein! 
- Beispiel: 
...
- Wir müssen Programmierer wenigstens warnen, wenn sie sowas machen
  wollen 
- Wir führen lieber neue Schlüsselwort ein! 

#+CAPTION: Neue Schlüsselworte als Warnung! 
#+ATTR_LaTeX: :width 0.25\linewidth
#+NAME: fig:doublepalm
[[./figures/doublefacepalm.png]]
...
#+BEGIN_SRC java :exports code :results output :classname XXX
  ...
  class B extends A implements bla, blub {
      public void blub() { System.out.println("BLUB"); }
  }
...
- Wesentliche Unterschied:
  - Verzicht auf Mehrfachvererbung und Imitation durch Interfaces
    - In neueren  Versionen gibt aber selbst Java zu, dass das Murks
      ist
  - Misstrauensmodell und feingranulare Zugriffskontrolle auf Daten,
    Methoden, Klassen
    - Bis hin zu ~private~ -- Nutzen stark strittig
