...
- Deklaration einer Klasse: ~class~
  - Erschafft eines neuen Datentyp
...
(in einer dynamisch typisierten Sprache nicht notwendig, eigentlich
gibt es ja auch keine Variablen in Python); sie werden
...
- Solche Methoden heißen /überladene Methoden/
  - Präziser: Der /Name/ der Methode ist überladen; aber hier wird
...
\emph{lokaler} und \emph{Wert}) verschwunden und \texttt{s} hat nach wie vor die
vorherigen Werte. 
#+END_LaTeX
...
Im Endeffekt sehr ähnlich zu Python! Im Mechanismus anders. 
...
|                               | Objektbezogen                                               | Klassebezogen/statisch                  |
|-------------------------------+-------------------------------------------------------------+----------------------------------------------|
...
| Attribut wird vernichtet      | wenn Objekt vernichtet wird (keine Referenz mehr existiert) | wenn Klasse entladen (meist: Programmende)   |
| Konstruktur                   | bei Objekterzeugung (~new~)                                 | bei Laden der Klasse                         |
| Zugriff auf Datenattribut ~d~ | ~obj.d~ oder ~this.d~                                       | ~Klassenname.d~                              |
| Aufruf einer Methode ~d~      | ~obj.m()~ oder ~this.m()~                                   | ~Klassenname.m()~                            |
...
class Artikel {
    ...
    public Artikel(String _an) {
        this.artikelnummer = _an;
    }
}

class Book extends Artikel {
    String ISBN;
    public Book(String _an, String _isbn) {
        // das hier verletzt DRY:
        this.artikelnummer = _an;
    ...
    public String toString() {
        return this.artikelnummer + " -- "  + this.ISBN;
    }
}

Book b = new Book ("1234", "abc");
System.out.println(b);
#+END_SRC

** Nein! Konstruktor von Article? 

- Es gibt keinen default-Konstruktor für Artikel
- Unklar, wie der existierende Konstruktor mit Parametern versorgt
  werden muss! 
...
class Article {
    String artikelnummer;
    public Article() { 
        this.artikelnummer = "Kein sinnvoller Wert";    }
    public Article(String _an) {
        this.artikelnummer = _an;
    }
}
...
- Verfügt die Oberklasse über einen default-Konstruktor (= ohne
  Parameter aufrufbar), so wird dieser benutzt
  - Unterklasse muss den default-Konstruktur nicht explizit aufrufen 
- Gibt es keinen solchen default-Konstruktor in der Oberklasse, so muss
  der Konstruktor  der Unterklasse einen passenden
  Oberklasse-Konstruktor explizit aufrufen
  - Mit Schlüsselwort ~super~ Zugriff auf Methode der Oberklasse
  - Konvention: Aufruf *muss* erste Anweisung im Konstruktor der
    Unterklasse sein
  - Passend: Nach Regeln für überladene Operatoren ausgewählt 
...
- Einfache Typen sind aber keine Klassen
  - ~int~, ~char~, ...
- /Wrapper/-Klasse: einen normalen Wert in ein passendes Objekt
  einwickeln
...
Klassenhierarchien, die nur Einfachvererbung nutzen, unterscheiden sich
kaum (zwischen Python und Java)  
...
- Dadurch entsteht kein ~Book~, keine ~ISBN~ aus dem Nichts!
...
** Typecast wenn falsches Objekt? Dynamisch? 
...
class Cover extends Article { /*...*/ }

class X {
    public static Article erzeuge() {
	if (heute ist Montag)
	    return new Book();
	else
	    return new Cover();
...
/* das klappt je nach Wochentag; statisch nicht entscheidbar */
Book b = (B) (new Article());
#+END_SRC 
...
- Nutzen der Konstruktion /hochgradig/ umstritten
  ...
  - (Man braucht das, um Closures nachzubauen, die Java eigentlich
    nicht hat...)
  - (Und angeblich kann schnellerer Code erzeugt werden -- sehr
    übertrieben) 
...
- Problem: Zwei Oberklassen definieren beide ein Attribut ~int x~
...
- Klasse ~C~ müsste alle Methoden selbst implementieren
  - Wenn Name mehrfach auftritt (~m~) ist nicht schlimm: wird ohnehin
    neu implementiert! 
...
- Die unbestrittenen Vorteile erkauft man sich durch
