...
- Schließlich: Setze die Lösung aus den Einzelteilen zusammen
  - Einzelteile in verschiedenen Kontexten nutzbar

Sog. */prozedurale/ funktionale Dekomposition/*
...
- Wir müssen uns überlegen:
  ...
  - Wie finden wir eine /gute/ Zerteilung eines Problems in
    Teilprobleme? 
...
- Es wird *eine* Anweisung ausgeführt: Die Definition der Funktion und
  das Binden an den Namen  ~meine_funktion~
  - Das ist die einzige Zustandsänderung! 
- Die Anweisungen ~x = 42~ und ~y = x/2~ werden *zunächst nicht* ausgeführt!
...
** Starke Analogie: Name, Funktionsname 
...
*** Unterschiedliche Notation für Namen  und Funktionen?          :dropslide:

Das hat letztlich pragmatische und auch Geschmacksgründe. Tatsächlich
gibt es Programmiersprachen (etwas Lisp), bei denen diese Analogie noch
viel enger ist und auch in der Syntax kaum ein Unterschied in der
Definition eines Namens für Werte   oder eines Funktionsnamens liegt. 
...
#+BEGIN_SRC python
%%tutor -t 
# hier wird f definiert: 
def f():
    x = 42
    y = 17

z = "vorher"
# hier wird f aufgerufen: 
f()
z = "nachher"
#+END_SRC
...
- Aufruf der Funktion: Programm mit den Anweisungen der Funktion
  fortsetzen
  - Vorstellung: Der /Aufruf/ der Funktion wird durch die
    /Anweisungen/ der Funktion ersetzt
  - Diese Anweisungen werden  dann ausgeführt 
- Nach Ausführung der letzten Anweisung der Funktion: Mit der
  Anweisung nach dem Aufruf weitermachen
  - Der Funktionsaufruf /ist beendet/ oder /kehrt zurück/
...
** Aufruf einer Funktion: Ersetzung 
...
*** Aufruf ersetzt (ACHTUNG! Nicht korrekt!) 
...
Die Namen ~x~ und ~y~ verhalten sich in diesen beiden Varianten
unterschiedlich! 

*** UEBUNG WArum sit das nur eine grobe Entsprechung

Wo sind die Unterschied? Zumindest einer klar: Im Zustand fehlt der
Eintrag für den Funktionsnamen f . Und die Namen  x und y sind
nicht im Scope. 
...
- Also stimmt obige Definition noch nicht ganz! 
- Beispiel: 
...
Werten.  Namensräume gruppieren Namen.

Während des Ablaufs eines Programs existieren in der Regel mehrere
Namensräume. Sie werden erzeugt und vernichtet. 
...
** Namensraum: Namen hinzufügen? 

- Ausgangspunkt: Aufruf einer Funktion als Ersetzung des Aufrufs durch
  Funktionsanweisungen zu erklären ist so falsch nicht
- Problem dabei: Die Namen innerhalb der Funktion
- Idee: Ergänze die von der Funktion benutzen Namen um den Namensraum
  - Hier vereinfacht: Hier setzen Namen der Funktion und den Namen des
    durch den Aufruf der Funktion erzeugten Namensraums gleich :-) 

** Aufruf einer Funktion: Ersetzung mit Namensraum? 
...
*** Aufruf ersetzt (ACHTUNG! Immer noch nicht korrekt!) 
...
# Ersetzung von f() mit Pseudonamen:  
f_x = 42
f_y = 17 
#+END_SRC
...
- Diese Namen ~f_x~, ~f_y~ gibt es ja nicht wirklich 
- Wir müssten sie also nach der Ausführung des ersetzen Codes wieder
  vernichten 
...
- Damit aber offene Frage: *Wo* kann denn eigentlich auf solche Namen
  eines Namensraum zugegriffen werden? 
...
- Information, wo das Programm nach Ende der Funktion fortgesetzt
  wird.
  - Das kann nicht statisch bekannt sein; die Funktion könnte ja von
    verschiedenen Stellen aus benutzt werden
...
*** Pingo 
#+BEGIN_SRC python :exports code
pingo_title = "Was ist der Wert von x am Ende? " 
...
* Rückgabewert 
** Rückgabewert 
- Benötigt: Mechanismus, um Wert aus Funktionsaufruf an den aufrufenden
...
2. [@2] Nach dem Aufruf und nach der Ausführung der Anweisung ~return~
   (Abbildung [[fig:pt-pi-return-value]]) wird im Namensraum von ~pi~ ein
   /scheinbarer/ Name  ~return value~ angelegt. Dieser Name zeigt
...
#+CAPTION: Nach Berechnung des Rückgabewerts, Anlegen des scheinbaren Namens ~return value~
#+ATTR_LaTeX: :width 0.75\linewidth
#+NAME: fig:pt-pi-return-value
[[./figures/pt-pi-2.png]]

3. [@3] Aber der /*Wert*/ dieses scheinbaren Namens bleibt bei der
   ...
   Im Beispiel hier ist das die Zuweisung an den Namen  ~x~. Als
...
** Funktion mit ~return~: Beispiel mit Code-Ersetzung 

#+BEGIN_SRC python
def pi():
    return 3.14159296

# Ersetze den Aufruf durch den Rückgabewert
x = 3.14159296
#+END_SRC


** Funktion mit ~return~: Beispiel 2 
...
# Ersetze den Aufruf durch die Anweisungen der Funktion
# Mit Namensraum vorangestellt:  
pi2_nicht_pi = 2.14159296
pi2_return_value = 1 + pi2_nicht_pi 
# Setze pi2_return_value für den Aufruf ein: 
x = pi2_return_value 
# Vernichte die Pseudonamen: pi2_nicht_pi ; pi2_return_value 
#+END_SRC
...
- Bei der Ausführung einer Funktion wird ein neuer Namensraum für die
  Funktion angelegt 
  - Dieser Namensraum wird nach Ende der Funktion wieder vernichtet
  - Namen, an die innerhalb der Funktion zugewiesen wird, existieren
...
- Ersetze in der aufrufenden Anweisung den Aufruf der Funktion
  ...
  - Fehlt ein expliziter Rückgabewert, so nimm ~None~ als besonderen
    Wert an
  - Wird der Wert der Funktion nicht benutzt, so ist der Rückgabewert
    egal 
...
- Funktionen können ja Namen benutzen
  ...
  - Anweisungen der Funktion weisen Werte zu 
- Wie wäre es, wenn innerhalb der  Funktion benutzte Namen /*von außen*/ mit Werten
  versorgt werden könnten? 
  - An sich normale Namen im Scope der Funktion
  - Aber der (anfängliche) Wert wird durch aufrufenden Programmteil festgelegt? 
...
** Funktion mit Parameter: Aufrufsyntax 
...
funktionieren, wenn mit  Namen für Werte arbeitet?)
...
** Funktion mit Parameter: Beispiel mit Code-Ersetzung 
...
*** Aufruf ersetzt, Namen mit Namensraum notiert: 

#+BEGIN_SRC python
def f(a):
    return a+1

# Aufruf ersetzt durch folgende Code-Folge 
# Parameter der Funktion an Wert binden: 
f_a = 2 
f_return_value = f_a + 1 
x = f_return_value 
# Vernichte f_a, f_return_value 
#+END_SRC


** Zur Betonung: Code-Ersetzung NICHT hinschreiben! 

- Um es nochmals zu betonen: Sie schreiben die Code-Ersetzung
  natürlich *nicht* selbst auf!
  - Das macht der Interpreter
- Sonst wäre die Einführung von Funktionen völlig zwecklos
  - Sie müssten das an *jeder* Stelle machen, an der Sie eine Funktion
    aufrufen würden - *zwecklos* 
- Das ist nur zur Illustration des (ungefähren) Effekts gedacht
  - Details der Implementation in einem Interpreter sind anders 
...
** Funktion mit Parameter: Aufruf mit Namen 
...
*** Name  als Argument                                            :dropslide:


Auch wenn ein Name an eine Funktion übergeben wird, passiert
...
deutlich: ~a~ und ~y~  beziehen sich beide  auf den gleichen Wert. 

#+CAPTION: Formaler Parameter ist Name für Wert  
#+ATTR_LaTeX: :width 0.75\linewidth
#+NAME: fig:pt-fa-y
[[./figures/pt-fa-y.png]]

**** Formale Parameter werden NICHT zu Namen für Namen         :B_alertblock:
     :PROPERTIES:
     :BEAMER_env: alertblock
     :END:
 ...
 einen anderen Namen sein (das gibt es nicht). 
...
der Name  ~y~ des Aufrufers und der Name  ~x~ der Funktion auf das
gleiche Objekt. 
...
vielleicht nach call-by-reference, ist aber nicht das gleiche,
sondern etwas deutlich anderes. Wir kommen darauf noch zurück.


** Aufrufsemantik: Pass-by-assignment (2)  
...
Mehr dazu in der Vorlesung Programmiersprachen. 
...
#+RESULTS:
: Hallo Programmierung


** ~print~ vs. ~return~? 

Zur Klarstellung: 
- ~print~ ist eine Funktion wie viele andere, mit einem bestimmten
  Seiteneffekt: Wert wird ausgegeben (auf Bildschirm o.ä.)
- ~return~ ist ein Schlüsselwort; es sorgt dafür, dass *nach* dem
  Aufruf einer Funktion ein in der Funktion berechneter Wert noch
  zugewiesen werden kann
- Das hat nichts mit einander zu tun, wird trotzdem manchmal
  verwechselt (häufiger Klausurfehler) 


** ~print~ vs. ~return~: Beispiel 
...
#+BEGIN_SRC python :exports code
pingo_title = "Werte von a und b?" 
pingo_questions = ["6 und 11", "6 und 2", "6 und None", "5 und 2", "5 und 11",]
pingo_duration = "180"
...
** Type hints: Kommentare zu den vorgesehenen Typen 

Zusätzlich oft nützlich: 
- Kommentare, welche Typen für Parameter erwartet werden
- Welchen Typ eine Funktion als Ergebnis liefert 
- Sog. ~type hints~
  ([[https://www.python.org/dev/peps/pep-0483][PEP 483]],
  [[https://www.python.org/dev/peps/pep-0484][PEP 484]],
  [[https://www.python.org/dev/peps/pep-0526/][PEP 526]])

Beispiel: ~greeting~ erwartet String und liefert String 
