...
*** Namespaces: Beispiele				  :B_example:animate:
    :PROPERTIES:
    :BEAMER_env: example
    :END:
...
Verkürzend sprechen wir von /Scope einer Variable/ und meinen damit den
...
*** Anmerkung: Pythontutor			     :B_definition:dropslide:
    :PROPERTIES:
    :BEAMER_env: definition
    :END:
...
Namensraum eindeutig zuordenbar und haben nichts mit einandern zu tun. 
...
Begriff wird insbesondere verwendet, wenn die Funktionen keinen
Rückgabewert hat. 
...
- ~print~ kann eine /beliebige Anzahl/ an Parameter verarbeiten
  - Durch Komma
...
- Wir brauchen zunächst: ~end~
  - Gibt Zeichen an, mit dem ~print~ die Ausgabe abschliesst
  - ~default~: Zeilenumbruch 

*** default					       :B_definition:animate:
    :PROPERTIES:
    :BEAMER_env: definition
    :END:

Ein ~default~-Wert für einen Parameter wird benutzt, wenn kein anderer
Parameter explizit angegeben wird. 

** Weitere Aspekte von ~print~: ~end~
