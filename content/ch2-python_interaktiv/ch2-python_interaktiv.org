...
Ein Interpreter ist ein Programm, dass Programme (oder
...
Beispiele für Interpreter gibt es viele, wenn man der Begriff des
/eingegebenen Programms/ etwas weiter fasst. Tabellenkalkulationen
sind typische Beispiele: Man gibt es Programm ein, das aus Formeln
...
- Eine Zahl darf *einen* Dezimalpunkt einhalten 
- Eine Zahl darf mit einem Vorzeichen *beginnen* 
...
- Test muss in Anführungszeichen eingeschlossen sein 
...
** Ausdrücke für Text?						   :subslide:

Zahlen kann man addieren - was ist mit Text?  Erwartung? 
...
*** Semantik von  ~+~ bei  Texten 			       :B_definition:
    :PROPERTIES:
    :BEAMER_env: definition
    :END:

Sind $t_1$ und $t_1$ Texte, so ist $t_1 + t_2$ ein Text. Der /Wert/
von  $t_1 + t_2$ ist der Text $t_1$, an den unmittelbar der Text $t_2$
angehängt wird. 

** Andere Verknüpfungen für Text 				   :subslide:
...
- Offenbar sind Zahlen und Texte ja etwas unterschiedliches
  - Unterschiedliche Werte, unterschiedliche Verknüpfungen erlaubt,
    unterschiedliche Interpretation (~+~ bei Zahlen vs. bei Texten) 
...
Opertionen / vs. % (divmod machen wir, wenn wir Funktionen eingeführt
haben) 

** Literale  für ganze Zahlen					   :subslide:

- Üblicherweise bauen  Literale für ganze Zahlen aus arabischen
Ziffern: 0, 1, 2, ..., 4711, ... 
...
** Datentyp: Text 						   :subslide:

*** Datentyp: Text						  :B_example:
    :PROPERTIES:
    :BEAMER_env: example
    :END:


Text sind Aneinandereihungen einzelner Zeichen. Die leere Reihe ist
ebenfalls ein Text. Zeichen sind durch eine Aufzählung definiert (a,
b, c, ...). 

Zwei Texte können lediglich durch Addition miteinander verknüpft
werden. 

** Datentyp: ~bool~ 						   :subslide:

*** Datentyp: bool					       :B_definition:
    :PROPERTIES:
    :BEAMER_env: definition
    :END:
...
*** Datentyp: Ganze Zahlen (ergänzt)				  :B_example:
 ...
 Zwei Zahlen können mit ~<~, ~<=~, ~>~, ~>=~, ~==~, ~!=~ mit einander
 verglichen werden. Das Resultat eines Vergleichs ist ein Wert des
 Datentyps ~bool~. 


*** UEBUNG Vergleich bei Text 

Wie Vergleich man zwei Texte? Formale Definition, bei Einzelzeichen
...
** Beispiel: Vergleich arithmetischer Ausdrücke			   :subslide:
...
- Text und Zahl?
  - Multiplikation: Text mehrfach hintereinander
- Text, Zahl und ~bool~?
  - ???
- Zahl und Zahl, genauer: unterschiedliche Zahlen?
  - Ja... 

** Text und Zahl						   :subslide:
...
Infinite size of ints , keine unterschiedlichen Größen der intergers 
...
- Verknüpfungen zwischen zwei Werten produziert Wert mit Typ
...
Ein Programmiersprache heißt /typisiert/ wenn für jeden Wert und
...
- *Ausdrücke für sich verändern den Zustand nicht* 
- ??? 
...
- Python ist ein Beispiel für
  ...
  - eine typisierte Programmiersprache 
