...
- Interaktive Programmierung?
  - Die /Sprache/ wird ausgeführt? -- Sinnlos 
...
Ausführung werden unmittelbar dargestellt. 
...
#+BEGIN_LaTeX
Abbildung~\ref{fig:repl}  zeigt die Read-Evaluate-Print-Schleife eines
Interpreters. Beachten Sie die Analogie zu einem Maschinenmodell: Ein
Interpreter ist in gewissem Sinne eine (sehr abstrakte) Form einer
Maschine.   
#+END_LaTeX


** Read-Evaluate-Print (REPL)  - Start? 

Genauer: 
- Wir können den Inhalt einer Box beliebig lange editieren 
- /read/ startet erst, wenn *Run* gedrückt (oder Tastenkombination)
- Dann wird *gesamter* Inhalt der Box an Interpreter übergeben;
  /evaluate/ startet
  - Editieren des Box-Inhaltes während /evaluate/ beeinflusst
    Ausführung nicht! 
- Nach Ende von /evaluate/ erneut *Run* möglich 
...
- Verbindet man zwei arithmetische Ausdrücke durch ~+~, ~-~, ~*~ oder
  '/', so entsteht ein /binärer/ arithmetischer Ausdruck
...
- Warum erlaubt Python die Verwendung von sowohl einfachen wie
  ...
  - Hinweis: Was wäre der Fall, wenn Sie ein doppeltes
...
- Als /Syntax/ würde das funktionieren 
- Aber keine sinnvolle /Semantik/, also Fehler 
...
- Vorheriges Kapitel: Die Ausführung einer Anweisung verändert den
  Zustand 
- Sind Ausdrücke Anweisungen?
  - *Nein*! Es wird hier kein Zustand verändert!
  - (Bis auf ... - siehe später) 
...
- Offenbar sind Zahlen und Zeichenketten ja etwas unterschiedliches
  - Unterschiedliche Werte, unterschiedliche Verknüpfungen erlaubt,
    unterschiedliche Semantik von Verknüpfungen (~+~ bei Zahlen
    vs. bei Zeichenketten)  
...
Ein /Datentyp/ oder kurz /Typ/ (engl. /Type/) legt eine Menge
möglicher Werte fest. /Literale/ sind die konkreten Notationen
möglicher Werte.
...
definiert. Siehe auch [[https://docs.python.org/3.5/library/stdtypes.html#][Details]]. 
...
- Damit hat jeder Ausdruck (genauer: Wert des Ausdrucks) einen Typ
...
** Bemerkung: Starke vs. schwache Typisierung                      :noexport:
...
** Bemerkung: Starke vs. schwache Typisierung (2)                  :noexport:
