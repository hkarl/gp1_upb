...
#+BEGIN_SRC python :exports both :results output 
t = ("Hallo", "Prog", (1, 2, 3, 9, 8, 7) ) 
print(t) 
#+END_SRC 

#+RESULTS:
: ('Hallo', 'Prog', (1, 2, 3, 9, 8, 7))


** Tuples und Objekte: Visualisierung				   :subslide:
...
** Tuple und Objekte: Visualisierung (2)			   :subslide:
...
überraschendes. Im Beispiel sind die beiden Tuple ~("Hallo", "Prog")~
...
** Tuple und Objekte: Visualisierung (3)			   :subslide:
...
werden? Das ist ganz analog zur Diskussion in Kapitel 3; auch da haben
mehrere Namen auf den gleichen Wert verwiesen. 

Aber natürlich werden mehrere Referenzen auf die Zahl 1 bzw. die 2 gebraucht!
...
** Einfache Subskripts  -- Beispiele			  :subslide:subslide:
...
Das scheitert -- Aufteilung wäre unklar: 
...
*** Unterschiedliche Anzahl mit Hilfe                               :animate:
...
Häufiges Muster: Kopf eines Tuples abtrennen
...
*** UEBUNG Unpacking von Tuples                                    :noexport:
...
Oder wie bekommt man Kopf, Rumpf und Ende einer Schleie? 
...
pingo_title = "Ausgabe von print(t[t[2][0:1][0]-1:t[2][1]])?" 
pingo_questions = ["'Hallo', 'Prog'", "('Hallo', 'Prog')", 
                       "(1, 2, 3)", "('Prog', '1', '2')" ]
pingo_duration = "180"
...
** Wo ist ein Element in Tuple?					   :subslide:
...
*** UEBUNG Was passiert bei (1,2,3,1,2,3).index(2) ? 

** Notation bei ~index~?					   :subslide:
...
** Wieviele Elemente mit bestimmtem Wert?			   :subslide:
...
** Keine Zuweisung an Tuple-Elemente				   :subslide:
...
immer das referenzierte Objekt. Daher kann man sich hier eine kleine
Schlampigkeit im Ausdruck erlauben. 
...
#+BEGIN_SRC python :exports code
pingo_title = "Was gibt print(L.index(2)) aus?" 
...
** Veränderung von Listenelementen				   :subslide:
...
#+CAPTION: Tada: Write access to list elements works 
...
** Pingo: ~append~ oder ~extend~?				    :nolatex:
...
#+BEGIN_SRC python :exports code
pingo_title = "append oder extend?" 
...
*** ~remove~: Erstes Auftauchen eines Wertes entfernen 		    :animate:
...
** Liste von Liste kopieren					   :subslide:
...
Die sich nach diesem Beispiel einstellende Datenstruktur wird  in
...
Man spricht hier von einer sog. /flachen Kopie/ (shallow
copy). Die Alternative wäre eine /tiefe Kopie/ (deep copy). Was würde
das bedeuten? 
...
** Pingo: Kopie einer Liste von Listen				    :nolatex:
...
#+BEGIN_SRC python :exports code
pingo_title = "Ausgabe nach Kopie einer Liste von Listen" 
...
** Identität: Zwei Vergleiche					   :subslide:
...
** Liste als Funktionsparameter -- Beobachtung?			   :subslide:
...
1. Abbildung [[fig:pt-modify-list-1]] zeigt die Situation vor Aufruf der
   Funktion an. Es gibt, im globalen Namensraum, zwei Namen, einen für die
   Funktion ~modify_list~ und einen für die Liste ~l~.
2. Nach Aufruf der Funktion (Abbildung [[fig:pt-modify-list-2]]) gibt es,
   im Namensraum der Funktion, einen weiteren Namen ~ll~ für die /gleiche
...
3. Nach der Zuweisung zu dem zweiten Element (Abbildung
   ...
   natürlich auch nach wie vor durch die beiden Namen ~l~ und ~ll~
   referenziert.
4. Damit ist nach der Rückkehr aus der Funktion (Abbildung
   [[fig:pt-modify-list-4]]) auch diese Änderung mittels des
   Namens ~l~ sichtbar. Die Tatsache, dass der zweite Name
...
** Liste als Funktionsparameter -- Ergebnisse transportieren?	   :subslide:
...
** Aufzählen  oder abbilden?					   :subslide:
...
D = {}
D["name"] = "Holger"
D["berufung"] = "Prog-Vorleser" 
D["telefon"] = 605375
D[1] = 17 
D[42] = "Prog" 
D["Vorlesungen"] = ["Prog", "KMS", "Rechnernetze"]
print(D)
print(D[42])
#+END_SRC 
...
** ~dict~: Werte angeben, Kurznotation				   :subslide:
...
*** Visualisierung von ~dict~					  :dropslide:

Zur Veranschaulichung hier noch eine Visualisierung eines ~dict~:
Der Name ~D~ referenziert die Datenstruktur des ~dict~ (Abbildung
...
** ~dict~: Test auf enthalten-sein				   :subslide:
...
und ~D1~ sind Dicts, die auf die gleichen Objekte verweisen (aber das
...
Die Visualisierung (Abbildung [[fig:pt-nested]]) dieser Datenstruktur ist
dann schon etwas größer, aber sicherlich nicht mehr überraschend:
...
Eine kompaktere (und praktisch äquivalente) Darstellung liefert
Abbildung [[fig:pt-nested-compact]]. Hier werden die einzelnen Objekte für 
die Schlüssel oder Werte nicht separat dargestellt sondern in
kompakter Form in der enthaltenden Aufzählung repräsentiert. Wir
werden gelegentlich auch diese kompakte Darstellung benutzen. 

#+CAPTION: Eine verschachtelte, gemischte Datenstruktur (kompakte Darstellung)
#+ATTR_LaTeX: :width 0.75\linewidth
#+NAME: fig:pt-nested-compact
[[./figures/pt-nesting-compact.png]]
...
** Konvertierung: Syntax 

*** Syntax der Typkonvertierung				       :B_definition:
    :PROPERTIES:
    :BEAMER_env: definition
    :END:
...
- Vorheriges Kapitel: Hinweise, welche Typen eine Funktion erwartet
  oder liefert
- Wäre das bei komplizierten Typen nicht auch nützlich?
  - Beispiel: Liste über Paare komplexer Zahlen -- wie
    aufschreiben?
- Zur strukturierten Dokumentation, zur Überprüfung des Codes vor der
  Laufzeit
  - Sog. /static type checking/

** Hinweise auf Typen - Type Hints
 
- Type Hint Mechanismus leistet das
  - Abstrakte Beschreibung von Datentypen
  - Hinweis, welchen Type eine Funktion erwartet, ein Name
...
Vector = List[Tuple[complex, complex]]

def inner_vector_product(v: Vector) -> complex: 
    return sum(x*y for x,y in v)

vec = [(1,1), (2,2)] # type: Vector
vec2 = [(1,1), (2,2)]
vec3 = ["a", "b", "c"]

inner_vector_product(vec)
inner_vector_product(vec2)
inner_vector_product(vec3)
inner_vector_product(17)
#+END_SRC


** Type hints überprüfen 

- Manche IDEs machen das automatisch (z.B. PyCharm) 
- Sonst: Kommandozeile mit Kommando ~mypy~ 
  - Gibt Hinweise auf Typ-Probleme  
- Weitere Info: PEP 484, [[http://mypy-lang.org/index.html][mypy Website]]
...
** Python: eingebaute Funktionen				   :subslide:
...
** Python-Syntax: Klammern					   :subslide:
