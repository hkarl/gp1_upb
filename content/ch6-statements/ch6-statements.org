...
überflüssiges Wort ~then~, was in manchen Sprachen vorkommt. 
...
Insebs.: das ist ein AUSDRUCK und keine Anweisung, kann also an einen Namen zugewiesen werden. 
...
ebenfalls eindeutig, für einen Menschen aber ggf. viel schwerer zu lesen
...
** Beobachtung: Sichtbarkeit von Namen 
...
** Beobachtung: Sichtbarkeit von Namen (2)                         :subslide:
...
*** Name nach Schleife sichtbar                                     :animate:
...
- Terminiert das für alle Eingaben? 
...
** ~while~: Beispiel Suchschleife  (3) 				   :subslide:
...
*** Schleifenbedingung zusammensetzen				    :animate:
...
*** UEBUNG Beispiele für Invarianten und Zielbedingung 

Insbes. mit Prüfung nach Schleife, ob Zielbedningung tatsächlich gilt
oder nur die Invariante verletzt wurde! 
...
** Zuweisung an Aufzählung -- es passiert gar nicht! 		   :subslide:
...
pingo_title = "Ausgabe nach Zuweisung an Liste" 
pingo_questions = ["1, 2, 3", "1, 17, 3"]

%pingo
#+END_SRC 

*** UEBUNG Variationen davon als Übungsaufgabe? 

- Exponentialfunktion in Array ablegen, durch zwei geschachelet
  for-Schleifen? 
...
*** Beobachtung: Reihenfolge					    :animate:
...
aus dem zweiten Durchlauf der Schleife. Die beiden Namen ~key~ und
~value~ der Schleife verweisen dabei auf Werte aus dem Dictionary ~D~.
...
*** Hilfsfunktion: ~range~ 				       :B_definition:
    :PROPERTIES:
    :BEAMER_env: definition
    :END:
...
*** Also für ~range~?					    :animate:nolatex:


#+BEGIN_SRC python :exports code
pingo_title = "Parameter von range für Zaun?" 
...
*** UEBUNG Erathostenes ohne range schreiben! 


*** UEBUNG Schöner schreiben!

mit ~enumerate~ die Ausgabe schöner machen! Und nur die Primzahlen
...
Finden Sie Situationen, wo diese beiden Code-Abschnitte *nicht* die
gleiche Ausgabe haben. Hinweis: Was ist, wenn es andere Namen für die
...
** List comprehsion mit Wahrheitswerten 
...
** List comprehsion mit Wahrheitswerten: Reduzieren? 
...
** Damit: Suchschleife ganz pythonisch 

(Mit einer noch fehlenden Modifikation, siehe später)

#+BEGIN_SRC python :exports both :results output
def ungerade_in_liste(L):
    return any([x % 2 == 1 for x in L])
#+END_SRC 
...
Und eigentlich schreibt man das als List comprehension 

#+BEGIN_SRC python :exports both :results output
result = [item for item in items if item in tests]
#+END_SRC 
...
*** UEBUNG Was ist schneller? Konventionell oder Slicing? 
...
*** UEBUNG Beispiele für zip in for SChleifen 
